...
int main(void) {
    ...
    if (shm_id == -1) {
        fprintf(
            stderr,
            "[Fabryka][ERRN] Unable to create Shared Memory segment! (%s)\n",
            strerror(errno));
        return 3;
    }
    ...
    if (sh_data == (void *)-1) {
        fprintf(
            stderr,
            "[Fabryka][ERRN] Failed to attach to Shared Memory segment! (%s)\n",
            strerror(errno));
        return 4;
    }
    ...
    if ((errno = pthread_create(&s1_pid, NULL, station_1,
                                (void *)&station_data)) != 0) {
        fprintf(stderr, "[Fabryka][ERRN] Failed to create Station 1! (%s)\n",
                strerror(errno));
        return 5;
    }
    if ((errno = pthread_create(&s2_pid, NULL, station_2,
                                (void *)&station_data)) != 0) {
    ...
    if ((errno = pthread_join(s1_pid, NULL)) != 0) {
        fprintf(
            stderr,
            "[Fabryka][ERRN] Failed to wait for Station 1 to finish! (%s)\n",
            strerror(errno));
        return 6;
    }
    if ((errno = pthread_join(s2_pid, NULL)) != 0) {
        fprintf(
            stderr,
            "[Fabryka][ERRN] Failed to wait for Station 2 to finish! (%s)\n",
            strerror(errno));
        return 6;
    }
    ...
    while (true) {
        if (stations_work) {
            ...
            if (semop(sem_id, &sem_op, 1) == -1) {
                fprintf(stderr,
                        "[Fabryka][ERRN] Failed to perform semaphore "
                        "operation! (%s)\n",
                        strerror(errno));
                return (void *)1;
            }

            if (mag_data->a_count > 0 && mag_data->b_count > 0 &&
                mag_data->c_count > 0) {
                mag_data->a_count--;
                mag_data->b_count--;
                mag_data->c_count--;
